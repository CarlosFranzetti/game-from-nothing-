...
function startLevel(resetOnly) {
  if (!resetOnly) {
    ...
    game.pelletsTotal = game.pelletsLeft;
    // fruit and siren pacing scale with the maze so every board feels the same
    game.fruitAt = [Math.round(game.pelletsTotal * 0.30), Math.round(game.pelletsTotal * 0.65)];
    game.fruitSpawns = 0;
    game.tunnelRows = new Set();
    for (let r = 0; r < ROWS; r++) if (' .o'.includes(game.grid[r][0])) game.tunnelRows.add(r);
...
function eatAt() {
  ...
  if ((ch === '.' || ch === 'o') && !game.eaten.has(key)) {
    ...
    // fruit triggers
    if ((game.pelletsEaten === game.fruitAt[0] && game.fruitSpawns === 0) ||
        (game.pelletsEaten === game.fruitAt[1] && game.fruitSpawns === 1)) spawnFruit();
...
function ghostSpeed(gh) {
  ...
  if (game.tunnelRows.has(r) && (c <= 7 || c >= 20)) return BASE_SPEED * spec.tunnel; // tunnel slow
...
function updateSiren() {
  ...
  if (anyDead) {
    ...
    const frac = game.pelletsLeft / (game.pelletsTotal || 1);
    const stage = frac > 0.72 ? 0 : frac > 0.45 ? 1 : frac > 0.2 ? 2 : 3;
